...
//v2 improves and robot manipulator movement, manipulator no longer touches conveyor belt during pickup (stops servos and moves slowly near near conveyor belt), manipulator has middle positions
//when moving to drop object in colour bin and when returning to its home position from a drop (this reduces force and speed of robot)
...
int16_t theta_1 = 90, theta_2 = 90, theta_3 = map(-65, 0, -90, 180, 90), theta_4 = map(25, 0, 90, 90, 180); //theta for joints 3 and 4, mapped from actual angle with respect to link to servo movement placement
...
const uint16_t j1 = 5, j2 = 6, j3 = 9, j4 = 10, claw = 11; //servo joints pin

Adafruit_TCS34725 tcs = Adafruit_TCS34725(TCS34725_INTEGRATIONTIME_700MS, TCS34725_GAIN_1X); //colour sensor

void setup() {
  Serial.begin(9600); //set serial print baud rate to 9600
  joint1.attach(j1);  //attaches joint1 servo to pin 5
  joint2.attach(j2);  //attaches joint2 servo to pin 6
  joint3.attach(j3);  //attaches joint3 servo to pin 9
  joint4.attach(j4);  //attaches joint4 servo to pin 10
  clawservo.attach(claw);  //attaches the claw servo to pin 11
...
void initial_position(){ //set robot arm to initial (home) position (x = 20.1961, y = 0, z = 36.6791)
  ...
  for(; theta_2 < 85; theta_2 += 5){
    joint2.write(theta_2);
    delay(50);
  }
  joint2.write(joint1_2_initial_position - 2);
  delay(100);
  ...
  delay(100);
  for(; theta_1 < 85; theta_1 += 5){
    joint1.write(theta_1);
    delay(50);
  }
  for(; theta_1 > 95; theta_1 -= 5){
    joint1.write(theta_1);
    delay(50);
  }
...
void right_drop(){ //have robot got to inital position and then to location above the rightmost (blue) bin  (x = 20.9520, y = 20.9520, z = 10.2992)
  initial_position();
  theta_1 = 135;
  theta_2 = 68;
...
void left_drop(){ //have robot got to inital position and then to location above the leftmost (red) bin  (x = 20.9682, y = 17.5944, z = 7.2284)
  initial_position();
  theta_1 = 50;
...
void center_drop(){ //have robot go to the inital position and then to the center (yellow) bin (x = 19.3792, y = 0, z = 12.0952)
  initial_position();
  theta_2 = 100;
